<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib prefix="c"  uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<c:set var="pageTitle" value="Complaint #${complaint.id}"/>
<c:set var="ctx" value="${pageContext.request.contextPath}"/>

<jsp:include page="/jsp/common/header.jsp"/>
<jsp:include page="/jsp/common/navbar.jsp"/>

<main class="container">
    <div class="mb-3">
        <a href="${ctx}/user/my-complaints" class="text-muted small">
            <i class="bi bi-arrow-left"></i> Back to My Complaints
        </a>
    </div>

    <div class="row g-4">
        <div class="col-lg-8">
            <div class="card-scs p-4">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    <div>
                        <div class="text-muted small">Complaint #<c:out value="${complaint.id}"/></div>
                        <h4 class="mb-0"><c:out value="${complaint.title}"/></h4>
                    </div>
                    <div class="text-end">
                        <span class="badge status-${fn:toLowerCase(complaint.status)} fs-6">
                            <c:out value="${complaint.status}"/>
                        </span>
                        <c:if test="${complaint.isEmergency}">
                            <span class="badge emergency-badge">EMERGENCY</span>
                        </c:if>
                    </div>
                </div>

                <div class="row small text-muted mb-3">
                    <div class="col-sm-6">
                        <div>Submitted <fmt:formatDate value="${complaint.createdAt}" pattern="dd MMM yyyy, HH:mm"/></div>
                        <c:if test="${not empty complaint.location}">
                            <div><i class="bi bi-geo-alt"></i> <c:out value="${complaint.location}"/></div>
                        </c:if>
                    </div>
                    <div class="col-sm-6 text-sm-end">
                        <div>
                            Priority:
                            <span class="badge priority-${fn:toLowerCase(complaint.priority)}">
                                <c:out value="${complaint.priority}"/>
                            </span>
                        </div>
                        <c:if test="${not empty complaint.etaHours}">
                            <div>ETA: <strong><c:out value="${complaint.etaHours}"/> hours</strong></div>
                        </c:if>
                        <c:if test="${not empty complaint.department}">
                            <div>Dept: <c:out value="${complaint.department.name}"/></div>
                        </c:if>
                    </div>
                </div>

                <c:if test="${not empty complaint.summary}">
                    <div class="mb-3">
                        <div class="text-muted text-uppercase small">Auto-Summary</div>
                        <p class="mb-0 fst-italic"><c:out value="${complaint.summary}"/></p>
                    </div>
                </c:if>

                <div class="mb-3">
                    <div class="text-muted text-uppercase small">Description</div>
                    <p class="mb-0" style="white-space: pre-wrap;"><c:out value="${complaint.description}"/></p>
                </div>

                <c:if test="${not empty complaint.tags}">
                    <div class="mb-3">
                        <div class="text-muted text-uppercase small mb-1">Tags</div>
                        <c:forEach items="${fn:split(complaint.tags, ',')}" var="t">
                            <span class="badge bg-light text-dark border">#<c:out value="${fn:trim(t)}"/></span>
                        </c:forEach>
                    </div>
                </c:if>

                <hr>
                <h6 class="mb-3">Reply Thread</h6>

                <c:choose>
                    <c:when test="${empty replies}">
                        <p class="text-muted small">No replies yet.</p>
                    </c:when>
                    <c:otherwise>
                        <div class="d-flex flex-column gap-2">
                            <c:forEach items="${replies}" var="r">
                                <div class="card-scs p-3">
                                    <div class="small text-muted mb-1">
                                        <strong><c:out value="${r.user.name}"/></strong>
                                        <span class="badge badge-role-${fn:toLowerCase(r.user.role)} ms-1">
                                            <c:out value="${r.user.role}"/>
                                        </span>
                                        <span class="ms-2">
                                            <fmt:formatDate value="${r.createdAt}" pattern="dd MMM, HH:mm"/>
                                        </span>
                                    </div>
                                    <div><c:out value="${r.message}"/></div>
                                </div>
                            </c:forEach>
                        </div>
                    </c:otherwise>
                </c:choose>

                <c:if test="${complaint.status != 'RESOLVED' && complaint.status != 'CLOSED'}">
                    <form method="post" action="${ctx}/user/reply" class="mt-3">
                        <input type="hidden" name="complaintId" value="${complaint.id}"/>
                        <div class="mb-2">
                            <label for="message" class="form-label small">Add a reply</label>
                            <textarea class="form-control" id="message" name="message" rows="3" required></textarea>
                        </div>
                        <button type="submit" class="btn btn-scs btn-sm">
                            <i class="bi bi-reply"></i> Send Reply
                        </button>
                    </form>
                </c:if>
            </div>
        </div>

        <div class="col-lg-4">
            <c:if test="${not empty recommendations}">
                <div class="card-scs p-3 mb-3">
                    <h6 class="text-muted text-uppercase small mb-2">
                        <i class="bi bi-lightbulb"></i> Suggested Solutions
                    </h6>
                    <ul class="mb-0 ps-3 small">
                        <c:forEach items="${recommendations}" var="r">
                            <li><c:out value="${r}"/></li>
                        </c:forEach>
                    </ul>
                </div>
            </c:if>

            <div class="card-scs p-3" id="similarCard">
                <h6 class="text-muted text-uppercase small mb-2">
                    <i class="bi bi-collection"></i> Similar Past Complaints
                </h6>
                <div id="similarList" class="small text-muted">Loading&hellip;</div>
            </div>
        </div>
    </div>
</main>

<script>
    (function () {
        var ctx = '${ctx}';
        var id  = ${complaint.id};
        fetch(ctx + '/api/similar?id=' + id)
            .then(function (r) { return r.ok ? r.json() : { results: [] }; })
            .then(function (data) {
                var el = document.getElementById('similarList');
                if (!data.results || data.results.length === 0) {
                    el.innerHTML = '<span class="text-muted">No similar complaints found.</span>';
                    return;
                }
                var html = '';
                data.results.forEach(function (c) {
                    html += '<a href="' + ctx + '/user/track-complaint?id=' + c.id + '" class="d-block py-1 border-bottom">' +
                            '<div class="fw-semibold text-dark">' + escapeHtml(c.title) + '</div>' +
                            '<div class="small text-muted">' +
                              '<span class="badge status-' + (c.status || 'PENDING').toLowerCase() + '">' + (c.status || '') + '</span> ' +
                              (c.location ? (' &middot; ' + escapeHtml(c.location)) : '') +
                            '</div></a>';
                });
                el.innerHTML = html;
            })
            .catch(function () {
                document.getElementById('similarList').textContent = 'Unable to load similar complaints.';
            });

        function escapeHtml(s) {
            if (s == null) return '';
            return String(s).replace(/[&<>"']/g, function (c) {
                return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c];
            });
        }
    })();
</script>

<jsp:include page="/jsp/common/footer.jsp"/>
